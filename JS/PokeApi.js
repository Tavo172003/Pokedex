...
fetch("https://pokeapi.co/api/v2/pokemon?limit=1025")
  ...
  .then((allPokemonData) => {
    ...
    [...tiposSet].sort().forEach((tipo) => {
      ...
      option.value = tipo;
      option.textContent =
        tipoTraducido[tipo] || tipo.charAt(0).toUpperCase() + tipo.slice(1);
            ...
            .then((info) => {
              ...
              return fetch(info.species.url)
                ...
                .then((speciesInfo) => {
                  ...
                  const descriptionEntry = speciesInfo.flavor_text_entries.find(
                  ...
                  return {
                    ...
                    stats: info.stats.map((s) => ({
                      name: s.stat.name,
                      value: s.base_stat,
                    })), // Extrae nombre y valor de las estadísticas.
                    cry: crySound,
...
function renderPokemones(resetCount = true) {
  ...
  const pokemonFiltrados = todosLosPokemones.filter((info) => {
    if (
      busqueda &&
      !(
        info.name.toLowerCase().includes(busqueda) ||
        String(info.id) === busqueda
      )
    )
      return false;
    ...
    if (gen !== "all") {
      ...
      if (
        genRanges[gen] &&
        !(id >= genRanges[gen][0] && id <= genRanges[gen][1])
      )
        return false;
    }
...
async function displayPokemonDetails(pokemon) {
  ...
  if (currentIndex > 0) {
    prevBtn.onclick = () =>
      displayPokemonDetails(todosLosPokemones[currentIndex - 1]);
  } else {
    ...
    if (node.evolves_to.length > 0) {
      node.evolves_to.forEach((nextEvolution) => {
        const evolutionTrigger =
          nextEvolution.evolution_details.length > 0
...
function getEvolutionMethod(details) {
  ...
  if (details.known_move)
    method += `con el movimiento ${details.known_move.name.replace(/-/g, " ")} `; // Con un movimiento conocido.
  if (details.min_happiness)
    method += `con ${details.min_happiness}+ de felicidad `; // Por felicidad.
  ...
  if (details.time_of_day && details.time_of_day !== "")
    method += `durante el ${details.time_of_day === "day" ? "día" : "noche"} `;
  if (details.party_species)
    method += `con ${details.party_species.name.replace(/-/g, " ")} en el equipo `; // Con una especie específica en el equipo.
  if (details.party_type)
    method += `con un tipo ${details.party_type.name} en el equipo `; // Con un tipo específico en el equipo.
  // Relación ataque/defensa.
  if (details.relative_physical_stats) {
    if (details.relative_physical_stats === 1)
      method += "si su ataque es mayor que su defensa ";
    else if (details.relative_physical_stats === -1)
      method += "si su defensa es mayor que su ataque ";
  ...
  if (details.gender)
    method += `(${details.gender === 1 ? "hembra" : "macho"}) `; // Por género.
  if (details.held_item)
    method += `con ${details.held_item.name.replace(/-/g, " ")} `; // Con un objeto equipado.
  if (details.location)
    method += `en ${details.location.name.replace(/-/g, " ")} `; // En una ubicación específica.
  if (details.turn_upside_down) method += "volteando la consola "; // Girando la consola (ej. Inkay).
  if (details.min_affection)
    method += `con ${details.min_affection}+ de afecto `; // Por afecto.
  if (details.max_health_stat) method += "con HP al máximo "; // Ejemplos, verificar API para nombres específicos.
  if (details.min_level_up)
    method += `al subir de nivel con ${details.min_level_up} `; // Ejemplos, verificar API para nombres específicos.
...
function getLocationsHtml(locationsData) {
  ...
  locationsData.forEach((encounter) => {
    ...
    const versions = encounter.version_details.map((v) =>
  ...
  uniqueLocations.forEach((versionsSet, locName) => {
    ...
    const formattedVersions = [...versionsSet]
...
function applyTypeColors() {
  document.querySelectorAll(".tipo-pokemon").forEach((element) => {
    ...
    if (tipo && typeColores[tipo]) {
      // Si el tipo existe y tiene un color definido:
